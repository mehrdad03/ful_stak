...
<div class="row container p-lg-0 mx-auto mx-lg-0" id="financial">
    <!-- ====== sidebar ====== -->
    ...
    <div class="col col-lg-10 p-0 px-lg-4">
        ...
        <section>
            ...
            <!-- payment table -->
            <div class="bg-secondary text-white w-100 rounded-4 overflow-hidden">
                <!-- table title -->
                <table id="table" class="w-100 responsive_table">
                    <!-- thead -->
                    <thead>
                    <tr class="text-center">
                        <th data-label="شماره پیگیری" class="p-4">شماره پیگیری</th>
                        <th data-label="شرح تراکنش" class="p-4">شرح تراکنش</th>
                        <th data-label="مبلغ" class="p-4">مبلغ</th>
                        <th data-label="ساعت" class="p-4">ساعت</th>
                        <th data-label="تاریخ" class="p-4">تاریخ</th>
                    </tr>
                    </thead>
                    <!-- tbody -->
                    <tbody class="">
                    @forelse($transactions as $item)
                        ...
                        <tr wire:key="{{$loop->index+1}}"
                            ...
                            <td data-label=": ساعت" class="p-3">{{\Morilog\Jalali\Jalalian::forge($item->created_at)->format('H:i')}}</td>
                            <td data-label=": تاریخ" class="p-3">{{\Morilog\Jalali\Jalalian::forge($item->created_at)->format('%A, %d %B 14%y')}}</td>
                        </tr>
    ...
    <!-- Modal -->
    @include('livewire.client.profile.order-item-model-financial')
</div>

@push('scripts')
    @script
    <script>
        window.addEventListener('items-modal', event => {
